...
function qdldl(A::SparseMatrixCSC{Tv,Ti};
    ...
    #factor the matrix
    factor!(workspace,logical,reg,is_psd)
...
function Base.:\(F::QDLDLFactorisation,b)
    return qdldl_solve(F,b)
end


function factor!(workspace::QDLDLWorkspace{Tf,Ti},logical,reg,is_psd) where {Tf<:AbstractFloat,Ti<:Integer}
...
function qdldl_solve(F::QDLDLFactorisation,b)
    x = copy(b)
    solve!(F,x)
    return x
end
...
function QDLDL_factor!(n,Ap,Ai,Ax,Lp,Li,Lx,D,Dinv,E,Lnz,etree,bwork,iwork,fwork,logicalFactor,reg,is_psd)
    ...
    if(!logicalFactor)
        ...
        D[1]     = Ax[1]
        D[1], E[1] = bound_from_zero(D[1], reg, is_psd)
    ...
    @inbounds for k = 2:n
        ...
        D[k], E[k] = bound_from_zero(D[k], reg, is_psd)
...
function bound_from_zero(x, reg, is_psd)
    if x > reg || (x < -reg && !is_psd)
        return x, 0.
    elseif x > 0 || is_psd
        return reg, reg - x
    else
